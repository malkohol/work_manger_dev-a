...
$edit = 1;	
		$edit_id = $_GET['edit'];
		$sql_products ="SELECT * FROM `mail_user` where id =  $edit_id";
...
?>
		...
		<div class="row">
			...
			<div class="col-lg-12">
							...
							<form id="user_form" >
								...
								<div class="form-group">
									...
									<textarea  name="comment" class="form-control" placeholder="comment" rows="3"><? echo $comment ?></textarea>
														...
														<div class="form-group">
									<label>Marketing szerepkör</label>
							<? echo  $statusz?> 
									<select name="statusz" name="statusz" class="form-control">
								<?
				for ($ia = 0; $ia <= count($_SESSION['mail_user'])-1; $ia++) {
				...
				echo "<option  value='".$ia."' ".$sel_var.">".$_SESSION['mail_user'][$ia]."</option>";
				}?>
									</select>
								</div>
