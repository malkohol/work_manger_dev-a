...
include($root.'/script/conn.php');
 	...
 	if(isset($_POST['user_dat'])){
 		...
 		$sql_var = "where nev like '%".$user_dat."%'  OR email like '%".$user_dat."%'   OR tel like '%".$user_dat."%' "; 
 	}else{
 		$sql_var = "";
 		}
 			...
 			$count_db = 0;
 				...
 				$sql_email ="SELECT * FROM  `user` $sql_var  limit 0,40";
...
for ($ib = 0; $ib <= count($_SESSION['user'])-1; $ib++) {
if($ib== $aktiv){
$aktiv_var = 	$_SESSION['user'][$ib];
	}	
}

echo "<div class='col-md-2'> ".$aktiv_var."</div>";
echo '<div class="col-md-2"> 
<button id="'.$id.'" class="myBtn btn btn-primary btn-md" type="button" onclick="open_user(this.id);"> Szerkesztés</button>';
echo "</div>";
